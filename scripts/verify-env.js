...
function checkEnvFile() {
    ...
    if (foundFiles.length === 0) {
        ...
        console.log(
            "   Please create .dev.vars with your environment variables.",
        );
        console.log(
            "   Copy .env.example to .dev.vars and fill in your values.",
        );
...
function validateEnvVars(envFile) {
    ...
    Object.keys(REQUIRED_ENV_VARS).forEach((key) => {
        ...
        if (
            !envVars[key] ||
            envVars[key] === `your-${key.toLowerCase().replace(/_/g, "-")}`
        ) {
            missing.push(key);
            allValid = false;
...
function checkWranglerConfig() {
    ...
    try {
        ...
        const config = JSON.parse(
            wranglerContent.replace(/\/\*[\s\S]*?\*\/|\/\/.*$/gm, ""),
        );
        ...
        if (config.d1_databases && config.d1_databases.length > 0) {
            console.log(
                `   - D1 databases: ${config.d1_databases.length} configured`,
            );
        ...
        if (config.r2_buckets && config.r2_buckets.length > 0) {
            console.log(
                `   - R2 buckets: ${config.r2_buckets.length} configured`,
            );
